"""
Provider Opt-in Test for codewithtony@gmail.com

Test the complete provider opt-in flow:
1. Create provider with codewithtony@gmail.com
2. Send opt-in email to provider
3. Wait for provider response (run on your terminal)
4. Process consent and trigger automation to send to thakurujwal13@gmail.com
"""
...
def cleanup_test_data(db, force=False):
    ...
    try:
        ...
        test_provider = fresh_db.query(ServiceProvider).filter(
            ServiceProvider.provider_id == "tony-provider-001"
        ).first()
...
def create_test_provider(db):
    """Create test provider with codewithtony@gmail.com"""
    ...
    provider = ServiceProvider(
        provider_id="tony-provider-001",
        company_name="Code with Tony",
        contact_email="codewithtony@gmail.com",
...
def get_existing_buyer(db):
    """Get existing buyer thakurujwal13@gmail.com"""
    buyer = db.query(BuyerCompany).filter(
        BuyerCompany.buyer_id == "real-optin-buyer-001"
    ).first()
    
    if buyer:
        print(f"✅ Using existing buyer: {buyer.buyer_id}")
...
def check_provider_response(db, provider_id):
    ...
    result = optin_service.check_provider_response(provider_id, platform_email="malaythakur1800@gmail.com")
...
def process_consent(db, provider_id):
    ...
    result = optin_service.process_consent(provider_id, from_email="malaythakur1800@gmail.com")
...
def run_optin_flow_test():
    ...
    print("=" * 70)
    print("PROVIDER OPT-IN TEST - Code with Tony")
    print("=" * 70)
    ...
    try:
        # Don't cleanup initially - check if provider exists
        cleanup_test_data(db, force=False)
        
        # Create provider
        provider = create_test_provider(db)
        
        # Get existing buyer
        buyer = get_existing_buyer(db)
        if not buyer:
            print("⚠️  Buyer not found - please ensure thakurujjwal13@gmail.com exists in database")
            return False
